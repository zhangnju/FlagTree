...
from triton._internal_testing import is_cuda, is_hip, is_hip_cdna3, is_musa
...
def test_tensor_descriptor_load(dtype_str, num_ctas, M_BLOCK, N_BLOCK, device):
    ...
    def alloc_fn(size: int, align: int, stream: Optional[int]):
        descriptor_size = 64 if is_musa() else 128
        assert size == descriptor_size * num_ctas
        assert align == descriptor_size
        assert stream == 0
...
def test_tensor_descriptor_store(dtype_str, num_ctas, M_BLOCK, N_BLOCK, device):
    ...
    def alloc_fn(size: int, align: int, stream: Optional[int]):
        descriptor_size = 64 if is_musa() else 128
        assert size == descriptor_size * (grid_m * grid_n) * num_ctas
        assert align == descriptor_size
        assert stream == 0
        return torch.empty(size, dtype=torch.int8, device=device)
...
def test_tensor_descriptor_functional_interface(dtype_str, device):
    ...
    def alloc_fn(size: int, align: int, stream: Optional[int]):
        descriptor_size = 64 if is_musa() else 128
        assert size == 2 * descriptor_size * (grid_m * grid_n)
        assert align == descriptor_size
        assert stream == 0
        return torch.empty(size, dtype=torch.int8, device=device)
...
def test_make_tensor_descriptor_matmul(num_stages, num_ctas, BLOCK_M, BLOCK_N, BLOCK_K, device):
    ...
    def alloc_fn(size: int, align: int, stream: Optional[int]):
        descriptor_size = 64 if is_musa() else 128
        assert size == 3 * descriptor_size * grid[0] * grid[1] * num_ctas
        assert align == descriptor_size
        assert stream == 0
        return torch.empty(size, dtype=torch.int8, device=device)
...
def test_make_tensor_descriptor_loop_carried(device):
    ...
    def alloc_fn(size: int, align: int, stream: Optional[int]):
        descriptor_size = 64 if is_musa() else 128
        assert size == descriptor_size * grid[0]
        assert align == descriptor_size
        assert stream == 0
        return torch.empty(size, dtype=torch.int8, device=device)
...
def test_tensor_descriptor_batched_gemm_2d_tma(device):
    ...
    if is_hip() or is_musa():
        # Insufficient shared memory for the larger block size
    ...
    def alloc_fn(size: int, align: int, stream: Optional[int]):
        ...
        descriptor_size = 64 if is_musa() else 128
        assert size == descriptor_size * 3 * (num_stages + 1) * grid[0]
        assert align == descriptor_size
        assert stream == 0
        return torch.empty(size, dtype=torch.int8, device=device)
...
def test_tensor_descriptor_batched_gemm_3d_tma(device):
    ...
    def alloc_fn(size: int, align: int, stream: Optional[int]):
        # TODO: should only need num_stages * 3 descriptors per SM
        descriptor_size = 64 if is_musa() else 128
        assert size == descriptor_size * 3 * grid[0]
        assert align == descriptor_size
        assert stream == 0
        return torch.empty(size, dtype=torch.int8, device=device)
    ...
    h = batched_gemm_3d_tma_kernel[grid](
        ...
        NUM_SMS,  #
        num_stages=num_stages, num_warps=8)
    torch.musa.synchronize() if is_musa() else torch.cuda.synchronize()
...
def test_tma_gather(X, Y, BLOCK_X, BLOCK_Y, dtype, y, device):
    ...
    torch.manual_seed(42)
    if dtype != torch.int8:
        input = torch.rand((X, Y), dtype=dtype, device='cpu').to(device)
    else:
        input = torch.arange(X * Y, dtype=dtype, device='cpu').to(device).reshape(X, Y)
...
def test_tma_gather_dot_pipeline(BLOCK_M, BLOCK_N, BLOCK_K, K, device):
    ...
    a = torch.arange(BLOCK_M * K, device='cpu').to(device).reshape(BLOCK_M, K).float()
    b = torch.arange(K * BLOCK_N, device='cpu').to(device).reshape(K, BLOCK_N).float()
...
def test_tma_scatter(X, Y, BLOCK_X, BLOCK_Y, dtype, y, device):
    ...
    torch.manual_seed(42)
    input = torch.arange(BLOCK_X * BLOCK_Y, dtype=dtype, device='cpu').to(device).reshape(BLOCK_X, BLOCK_Y)

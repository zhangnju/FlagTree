...
class BaseBackend(metaclass=ABCMeta):
    ...
    def refine_specialization(self, bound_args, params, specialization, *, jit_function=None):
        """Optionally refine the native JIT specialization result on MUSA backend.

        The native specialization fast path intentionally only distinguishes
        16-byte divisibility. Backends with additional hardware legality
        requirements may override this hook to add finer cache-key classes and
        argument attributes without changing user kernels. ``jit_function``
        lets a backend scope refinements to kernels that use the corresponding
        feature instead of changing cache behavior for every kernel.
        """
